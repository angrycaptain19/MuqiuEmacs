...
class _BreakOutsideLoop(SyntaxRule):
    ...
    def is_issue(self, leaf):
        in_loop = any(
            block.type in ('for_stmt', 'while_stmt')
            ...
            if error is None:
                if second.type == 'dictorsetmaker':
                    if self._normalizer.version < (3, 8):
                        ...
                        error = 'dict display' if second.children[1] == ':' else 'set display'
                elif first == "{" and second == "}":
                    error = 'literal' if self._normalizer.version < (3, 8) else "dict display"
                elif first == "{" and len(node.children) > 2:
                    error = 'literal' if self._normalizer.version < (3, 8) else "set display"
                elif first in ('(', '['):
