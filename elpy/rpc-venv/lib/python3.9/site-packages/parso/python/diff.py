...
def _ends_with_newline(leaf, suffix=''):
    ...
    typ = leaf.token_type.lower() if leaf.type == 'error_leaf' else leaf.type
...
def _flows_finished(pgen_grammar, stack):
    ...
    """
    return all(
        stack_node.nonterminal
        not in ('if_stmt', 'while_stmt', 'for_stmt', 'try_stmt')
        ...
        for node in nodes:
            ...
            try:
                ...
                if n.type in ('async_funcdef', 'async_stmt'):
                    n = n.children[-1]
                suite_node = n.children[-1] if n.type in ('classdef', 'funcdef') else c[-1]
