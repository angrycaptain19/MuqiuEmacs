...
def _create_params(parent, argslist_list):
    ...
    if first.type in ('name', 'fpdef'):
        ...
        children = [first] if first.type == 'tfpdef' else first.children
        new_children = []
        start = 0
...
class IfStmt(Flow):
    ...
    def is_node_after_else(self, node):
        ...
        """
        for c in self.children:
            if c == 'else' and node.start_pos > c.start_pos:
                return True
        else:
            return False
...
class ImportFrom(Import):
    ...
    def _aliases(self):
        ...
        return {alias: name for name, alias in self._as_name_tuples()
    ...
    def _as_name_tuples(self):
        ...
        as_names = last.children[::2] if last.type == 'import_as_names' else [last]
...
class ImportName(Import):
    ...
    def _aliases(self):
        ...
        """
        return {alias: path[-1] for path, alias in self._dotted_as_names()
                        if alias is not None}
...
class ExprStmt(PythonBaseNode, DocstringMixin):
    ...
    def get_rhs(self):
        ...
        if node.type == 'annassign':
            node = node.children[3] if len(node.children) == 4 else node.children[1]
        return node
